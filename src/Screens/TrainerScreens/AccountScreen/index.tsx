...
import { Text, View, TouchableOpacity, StyleSheet, Image, ScrollView, Pressable, Platform } from "react-native";
...
const AccountScreen = ({ navigation }: PropsInterface) => {
  ...
  const UpdatePassword = async () => {
    ...
    const result: any = await updatePassword({ id: userMeData?.user._id, data: body });
  ...
  if (isUserMeLoading) <FullPageLoader />;
  return (
    <Container>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ alignItems: "center", marginTop: 10 }}>
          <Pressable disabled={!!userMeData?.user.accountVerified} style={{ flexDirection: "row", padding: 10, backgroundColor: Colors.lightGrays, columnGap: 15, borderRadius: 10 }} onPress={() => navigation.navigate("TrainerVerification")}>
            <Typography>{!userMeData?.user.accountVerified ? "Apply For verification" : userMeData?.user.accountVerified.toUpperCase()}</Typography>
            <AntDesign name={userMeData?.user.accountVerified ? "checksquare" : "closesquare"} size={20} color={userMeData?.user.accountVerified ? Colors.darkGreen : Colors.redColor} />
          </Pressable>
        </View>
        <View style={{ alignItems: "center" }}>
          <View
        ...
        <View style={{ alignItems: "center", marginVertical: 5 }}>
          <Pressable
            onPress={() => {
              if (userMeData?.stripe.card) navigation.navigate("TrainerWallet");
        ...
        <View style={{ backgroundColor: Colors.black, alignItems: "center", borderRadius: 10, marginVertical: 5 }}>
          ...
          >
            <View style={{ flexDirection: "row" }}>
              <Entypo name={"key"} size={25} color={"#fff"} />
              <Typography style={{ marginLeft: 10 }} color="white">
                Change Password
              </Typography>
            </View>
          ...
          {ChangePassword && (
            ...
            >
              ...
              <TextInput style={styles.inputPassword} label="New Password" placeholder="Enter Password" secureTextEntry value={newPassword} onChangeText={setNewPassword} />
              <TextInput style={styles.inputPassword} label="New Password" placeholder="Enter Password" secureTextEntry value={confirmPassword} onChangeText={setConfirmPassword} />
        ...
        <View style={{ alignItems: "center", marginVertical: 5 }}>
          <Pressable
            onPress={() => navigation.navigate("UpdateProfessioninfo")}
            style={{
              width: "100%",
              alignItems: "center",
              backgroundColor: "#000",
